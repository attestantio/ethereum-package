...
def generate_certs(
    plan, dirk_service_names, vouch_client_name="vouch-client", cluster_id=None
):
    ...
    prefix = "dirk-{0}".format(cluster_id) if cluster_id != None else "dirk"
    ...
    result = plan.run_sh(
        name="generate-{0}-certs".format(prefix),
