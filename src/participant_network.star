...
def launch_participant_network(
    ...
    if has_vouch_participant:
        ...
        if dirk_signing_threshold < 1:
            fail(
                "dirk_signing_threshold must be at least 1, got {0}".format(
                    dirk_signing_threshold
                )
            )
        if dirk_signing_threshold > dirk_peer_count:
            fail(
                "dirk_signing_threshold ({0}) cannot exceed dirk_peer_count ({1})".format(
                    dirk_signing_threshold,
                    dirk_peer_count,
                )
            )
        ...
        for vp in vouch_participants[1:]:
            if (
                vp.dirk_peer_count != dirk_peer_count
                or vp.dirk_signing_threshold != dirk_signing_threshold
                or vp.dirk_image != dirk_image
            ):
                plan.print(
                    "WARNING: vouch participants have inconsistent Dirk settings; using first vouch participant's config (peer_count={0}, threshold={1}, image={2})".format(
                        dirk_peer_count,
                        dirk_signing_threshold,
                        dirk_image,
                    )
